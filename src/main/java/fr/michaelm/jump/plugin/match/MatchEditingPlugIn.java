...
public class MatchEditingPlugIn extends AbstractPlugIn implements ActionListener {
    ...
    private Layer findTargetLayer(Layer links, String attribute, LayerManager layerManager) {
        ...
        for (Feature f : links.getFeatureCollectionWrapper().getFeatures()) {
            int fid = f.getInteger(index);
            for (Layer lyr : layerManager.getLayers()) {
                if (lyr == links) continue;
                for (Feature f2 : lyr.getFeatureCollectionWrapper().getFeatures()) {
                    if (fid == f2.getID()) {
                        return lyr;
                    }
                }
    ...
    public void actionPerformed(ActionEvent e) {
        if ("link".equals(e.getActionCommand())) {
                Collection<Feature> selectedSource = workbenchContext
                        .getLayerViewPanel()
                        .getSelectionManager()
                        .getFeaturesWithSelectedItems(sourceLayer);
                Collection<Feature> selectedTarget = workbenchContext
                        .getLayerViewPanel()
                        .getSelectionManager()
